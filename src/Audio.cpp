...
AudioSource &AudioSource::loop(bool doLoop)
{
    alCall(alSourcei, handle, AL_LOOPING, doLoop);
    return *this;
}

AudioSource &AudioSource::updateState()
{
    alCall(alGetSourcei, handle, AL_SOURCE_STATE, &state);
    return *this;
}

AudioSource &AudioSource::setBuffer(ALint buffer)
{
    alCall(alSourcei, handle, AL_BUFFER, buffer);
    return *this;
}

AudioSource &AudioSource::setPosition(vec3 p)
{
    alCall(alSource3f, handle, AL_POSITION, p.x, p.y, p.z);
    return *this;
}

AudioSource &AudioSource::setVelocity(vec3 v)
{
    alCall(alSource3f, handle, AL_VELOCITY, v.x, v.y, v.z);
    return *this;
}

AudioSource &AudioSource::setPitch(ALfloat pitch)
{
