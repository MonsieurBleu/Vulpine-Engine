...
{
    PhysicsMaterial(float friction = 0.0f, float restitution = 0.0f, float linearDamping = 0.0f,
...
class Collider
{
  public:
    ...
    std::optional<IntersectionInfo> getCollisionInfo(const glm::vec3 &position, const glm::vec3 &positionOther,
                                                     const glm::quat &rotation, const glm::quat &rotationOther,
                                                     const Collider *collider, bool continuous);

    virtual std::pair<glm::vec3, glm::vec3> getBoundingBox(const glm::vec3 &position,
                                                           const glm::quat &rotation) const = 0;

  protected:
    virtual std::optional<IntersectionInfo> getCollisionInfoAABB(const glm::vec3 &position,
    ...
    virtual std::optional<IntersectionInfo> getCollisionInfoOBB(const glm::vec3 &position,
                                                                const glm::vec3 &positionOther,
                                                                const glm::quat &rotation,
                                                                const glm::quat &rotationOther,
    ...
    virtual std::optional<IntersectionInfo> getCollisionInfoSphere(
    ...
    virtual std::optional<IntersectionInfo> getCollisionInfoCapsule(
        const glm::vec3 &position, const glm::vec3 &positionOther, const glm::quat &rotation,
        const glm::quat &rotationOther, const CapsuleCollider *collider, bool continuous) const
    {
        return std::nullopt;
    }
    virtual std::optional<IntersectionInfo> getCollisionInfoConvex(
        const glm::vec3 &position, const glm::vec3 &positionOther, const glm::quat &rotation,
...
class AABBCollider : public Collider
{
  public:
    ...
    std::optional<IntersectionInfo> getCollisionInfoAABB(const glm::vec3 &position, const glm::vec3 &positionOther,
                                                         const glm::quat &rotation, const glm::quat &rotationOther,
                                                         const AABBCollider *collider, bool continuous) const override;
    std::optional<IntersectionInfo> getCollisionInfoOBB(const glm::vec3 &position, const glm::vec3 &positionOther,
                                                        const glm::quat &rotation, const glm::quat &rotationOther,
                                                        const OBBCollider *collider, bool continuous) const override;
    std::optional<IntersectionInfo> getCollisionInfoSphere(const glm::vec3 &position, const glm::vec3 &positionOther,
                                                           const glm::quat &rotation, const glm::quat &rotationOther,
                                                           const SphereCollider *collider,
                                                           bool continuous) const override;
    std::optional<IntersectionInfo> getCollisionInfoConvex(const glm::vec3 &position, const glm::vec3 &positionOther,
                                                           const glm::quat &rotation, const glm::quat &rotationOther,
...
class SphereCollider : public Collider
{
  public:
    ...
    std::optional<IntersectionInfo> getCollisionInfoSphere(const glm::vec3 &position, const glm::vec3 &positionOther,
                                                           ...
                                                           const SphereCollider *collider,
                                                           bool continuous) const override;
    std::optional<IntersectionInfo> getCollisionInfoCapsule(const glm::vec3 &position, const glm::vec3 &positionOther,
                                                            const glm::quat &rotation, const glm::quat &rotationOther,
...
class ForceField
{
  public:
    ...
    template <typename... Args> static ForceField::Ref global(const ForceFieldFunction &forceFieldFunction)
    {
        return std::make_shared<ForceField>(nullptr, glm::vec3(), glm::identity<glm::quat>(), forceFieldFunction);
    }

    template <typename... Args> static ForceField::Ref make(Args &&...args)
    {
        return std::make_shared<ForceField>(std::forward<Args>(args)...);
    }

    ForceField(Collider *collider, glm::vec3 position = {}, glm::quat rotation = glm::identity<glm::quat>(),
    ...
    glm::vec3 getAcceleration(const glm::vec3 &position) const
    {
        return forceFieldFunction(position);
    }
    ...
    void setForceFieldFunction(const ForceFieldFunction &forceFieldFunction)
    {
        this->forceFieldFunction = forceFieldFunction;
    }

    static ForceFieldFunction uniformForceField(const glm::vec3 &acceleration)
    {
        return [acceleration](const glm::vec3 &position) { return acceleration; };
    }
    ...
    static ForceFieldFunction pointForceField(const glm::vec3 &center, float pull, float radius)
    {
        return [center, pull, radius](const glm::vec3 &position) {
            ...
            return glm::vec3((center - position) * std::lerp(pull, 0.0f, std::min(distance / radius, 1.0f)) / distance);
        };
    }
...
class RigidBody
{
  public:
    ...
    // todo move in the cpp
    template <typename... Args> static RigidBody::Ref make(Args &&...args)
    {
        return std::make_shared<RigidBody>(std::forward<Args>(args)...);
    }

    RigidBody(Collider *collider, float mass = 1.0f, PhysicsMaterial material = {}, int layer = 0,
              const glm::vec3 &position = {}, const glm::vec3 &velocity = {}, const glm::vec3 &acceleration = {},
              const glm::quat &rotation = glm::identity<glm::quat>(), const glm::vec3 &angularVelocity = {},
    ...
    {
        return isStatic() ? glm::vec3(0.0f, 0.0f, 0.0f) : velocity;
    }
    glm::vec3 getAcceleration() const
    {
        return isStatic() ? glm::vec3(0.0f, 0.0f, 0.0f) : acceleration;
    }
    glm::quat getRotation() const
    {
        return rotation;
    ...
    {
        return canRotate() ? angularVelocity : glm::vec3(0.0f, 0.0f, 0.0f);
    }
    glm::vec3 getTorque() const
    {
        return canRotate() ? torque : glm::vec3(0.0f, 0.0f, 0.0f);
    }
    float getMass() const
    {
    ...
    {
        return !isStatic() && collider->getColliderType() != Collider::ColliderType::AABB;
    }
...
class Trigger
{
  public:
    ...
    template <typename... Args> static Trigger::Ref make(Args &&...args)
    {
    ...
    Trigger(Collider *collider, glm::vec3 position = {}, glm::quat rotation = glm::identity<glm::quat>(),
            const TriggerEvent &onEnterEvent = nullEvent(), const TriggerEvent &onExitEvent = nullEvent(),
    ...
    void registerOnEnterEvent(const TriggerEvent &onEnterEvent)
    {
        this->onEnterEvent = onEnterEvent;
    }
    void registerOnExitEvent(const TriggerEvent &onExitEvent)
    {
        this->onExitEvent = onExitEvent;
    }
    void registerOnTickEvent(const TriggerEvent &onTickEvent)
    {
        this->onTickEvent = onTickEvent;
    }

    void triggerOnEnterEvent(RigidBody::Ref &rigidBody, float delta)
    {
        onEnterEvent(rigidBody, delta);
    };
    void triggerOnExitEvent(RigidBody::Ref &rigidBody, float delta)
    {
        onExitEvent(rigidBody, delta);
    };
    void triggerOnTickEvent(RigidBody::Ref &rigidBody, float delta)
    {
        onTickEvent(rigidBody, delta);
    };
